...
# the entity (person or company) that the user is asking about, used as context for future questions. Updated whenever a new entity is mentioned in the conversation
current_entity = None

# Small talk replies like greetings and casual conversation 
...
# ConversationChain for small talk (requires memory to maintain conversation history)
conversation = ConversationChain(llm=llm, memory=memory, prompt=chat_prompt, verbose=False)

# Identify if the user's message is small talk or a factual question to decide the flow of the conversation
...
# Identify the main entity (person or company) in the user's query to use as context for future questions
...
# format conversation history for question rewriting and answering
def format_history():
    ...
    for i in range(len(msgs)):
        ...
        # Add the role and message content to the history text
        history_text += f"{role}: {msgs[i].content}\n"

    # Remove the last extra newline and return
    return history_text.strip()


# Rewrite question to be standalone using conversation history and current entity as context
...
# Search for relevant information using  (DuckDuckGo + Wikipedia) and return snippets
...
#Fact-check the LLM's answer against the search snippets and return corrected answer if wrong
def verify_answer(user_query: str, llm_answer: str) -> str:
    """Check the LLM answer against search snippets and return correct concise answer."""
    snippets = free_search(user_query)
    if snippets.strip() == "I do not know.":
        return "I do not know."

    # Use LLM to select correct answer from snippets
    prompt = f"""
...
Instructions:
- Provide ONLY the correct short answer.
- Do NOT include extra titles, nicknames, or explanations.
- If answer is not in the information, reply "I do not know."
